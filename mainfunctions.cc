...
align_to_template(std::vector<Orf> &&orfs,
                  ...
                  bool ragged_ends)
{
        std::vector<float> template_max_scores;
        for (const AlignmentTemplate &tpl : templates) template_max_scores.push_back(nw_self_align_score(tpl.aas, BLOSUM62));
    
        auto align_to_template_worker = [&](Orf &&orf, ParseLog &log)->
            std::optional<GroupAlignment> {
                std::optional<GroupAlignment> result;
                if (orf.template_id == TemplateDatabase::NOT_FOUND) {
                    ++log.filter_no_matching_template;
                    return result;
                }

                const Aas &tpl = templates[orf.template_id].aas;
                const float template_max_score = template_max_scores[orf.template_id];

                Alignment aln;
                const int gapp = 4;
                float max_score = template_max_score;
                if (ragged_ends) max_score -= gapp * std::labs(static_cast<long>(tpl.size()) - static_cast<long>(orf.aas.size()));
                nw_align(orf.aas, tpl, BLOSUM62, gapp, aln);
                //nw_align_aas(orf.aas, p.aa_template, BLOSUM62, gapp, aln);
                if (aln.score / max_score < p.min_alignment_score) {
                    ++log.filter_bad_alignment;
                } else {
                    result = GroupAlignment();
                    result->barcode = std::move(orf.barcode);
                    result->template_id = orf.template_id;
                    result->umi_group_size = orf.umi_group_size;
                    result->alignment  = std::move(aln.aligned_query);
                    result->cdns.reserve(result->alignment.size());
                    auto cdn = orf.cdns.begin();
                    for (char c : result->alignment) {
                        result->cdns.push_back(c == '-' ? ' ' : static_cast<char>(*cdn++));
                    }
                }
                return result;
        };
...
/*
std::pair<std::vector<GroupAlignment>, std::vector<AlignmentTemplate>>
align_to_multiple_templates(vecvec<Orf> &&orfs,
                   ...
                   ParseLog &log) {
    std::vector<GroupAlignment>    alignments;
    std::vector<AlignmentTemplate> composite_templates;
    ...
    std::unordered_map<std::vector<size_t>,
                       ...
                       std::equal_to> template_index_lookup;

    auto align_to_templates_worker = [&](Orf &&orf, 
        const Aas &tpl,
        ParseLog &log)->std::optional<GroupAlignment> { 
        std::optional<GroupAlignment> result;

        const float max_score = nw_self_align_score(tpl, BLOSUM62);
    ...
    std::vector<size_t> partial_ids;
    std::vector<GroupAlignment> partial_alignments;
    for (size_t i=0; i<orfs.size(); ++i) {
        std::vector<Orf> &split = orfs[i];

        partial_ids.clear();
        partial_alignments.clear();
        for (size_t j=0; j<split.size(); ++j) {
            Orf &orf = split[j];
            const size_t template_id = orf.template_id;
            std::optional<GroupAlignment> alignment = align_to_templates_worker(
                std::move(orf),
                dbs[j]->get_aas(template_id),
                log
            );
            if (!alignment) break;
            partial_ids.push_back(alignment->template_id);
            partial_alignments.push_back(std::move(*alignment));
        }

        if (partial_alignments.size() != split.size()) { //at least one part didn't align
            ++log.filter_bad_alignment;
        } else {                                         //all parts aligned
            size_t template_id = composite_templates.size();
            auto [ii, success] = template_index_lookup.insert({partial_ids, template_id});
            if (success) {
                AlignmentTemplate tpl;
                for (size_t i=0; i<split.size(); ++i) {
                    tpl.label += dbs[i]->get_label (split[i].template_id) + " ";
                    tpl.aas   += dbs[i]->get_aas   (split[i].template_id);
                    tpl.cdns  += dbs[i]->get_codons(split[i].template_id);
                }
                if (!tpl.label.empty()) tpl.label.pop_back();
                composite_templates.push_back(std::move(tpl));
            } else {
                template_id = ii->second;
            }

            partial_alignments.front().template_id = template_id;
            for (size_t j=1; j<split.size(); ++j) partial_alignments.front() += partial_alignments[j];
            alignments.push_back(std::move(partial_alignments.front()));
        }
    }

    return std::make_pair(std::move(alignments), std::move(composite_templates));
}
*/
    ...
    auto align_to_multiple_templates_worker = [&](
        ...
        for (size_t i=0; i<orfs.size(); ++i) {
            ...
            if (ragged_ends) max_score -= dbs[i]->gap_penalty()
                                        * std::labs(static_cast<long long>( orfs[i].aas.size()) - 
                                                    static_cast<long long>(template_aas.size()));
...
/*
vecvec<Orf>
assign_templates(vecvec<Orf> &&orfs,
    ...
    auto assign_templates_worker = [&](std::vector<Orf> &&orfs, ParseLog &log)->
        std::optional<std::vector<Orf>>
    {
        assert (orfs.size() == dbs.size());
        std::optional<std::vector<Orf>> output;
        bool all_matched = true;
        for (size_t i=0; i<orfs.size(); ++i) {
            size_t id = dbs[i]->query(orfs[i].cdns);
            if (id == TemplateDatabase::NOT_FOUND) {
                all_matched = false;
                ++log.filter_no_matching_template;
                break;
            }
            orfs[i].template_id = id;
        }
        if (all_matched) output = std::move(orfs);
        return output;
    };
...
/*
std::vector< std::pair<GroupAlignment, std::vector<size_t>> >
align_multiple_templates(vecvec<Orf> &&orfs,
                 std::vector<std::shared_ptr<const TemplateDatabase>> &dbs,
                 const help::Params &params,
                 ParseLog &log) {
    vecvec<Orf> result;
    ...
    auto assign_templates_worker = [&](std::vector<Orf> &&orfs, ParseLog &log)->
        std::optional< std::pair<GroupAlignment, std::vector<size_t>> >
    {
        assert (orfs.size() == dbs.size());
        std::optional< std::pair<GroupAlignment, std::vector<size_t>>> output;
        std::vector<size_t> ids;
        bool all_matched = true;

        size_t id = 0;
        Alignment aln;
        GroupAlignment cumulative;
        for (size_t i=0; i<orfs.size(); ++i) {
            std::tie(id, aln) = dbs[i]->query_and_align(orfs[i].cdns);
            if (id == TemplateDatabase::NOT_FOUND) {
                all_matched = false;
                ++log.filter_no_matching_template;
                break;
        ...
        if (ids.size() != orfs.size()) return output;
        output = std::make_pair(std::move(cumulative), std::move(ids));
        return output;
    };
